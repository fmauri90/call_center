...
'''
@api_geo.route('/geo_condominium', methods=['GET'])
def geo_coder():
    if request.is_json:
        ...
        if "address" not in req_json:
            ...
            return res
        try:
            req_geo = requests.get('https://nominatim.openstreetmap.org/search?q={address}&format=json&polygon=0&addressdetails=1'.format(address=req_json.get('address')))
